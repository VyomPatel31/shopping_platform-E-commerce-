...
import {
  Loader2, Plus, Edit, Trash2, X, Save, Upload, Link2,
  Package, IndianRupee, ShoppingCart, Users, TrendingUp, ClipboardList
} from 'lucide-react';
...
const AdminDashboard: React.FC = () => {
  ...
  const onSubmit = async (data: ProductFormData) => {
    ...
    try {
      ...
      if (imageMode === 'upload') {
        ...
        imagesArray = typeof data.images === 'string'
  ...
  const handleOrderStatusChange = async (orderId: string, status: string) => {
    try {
      ...
      if (status === 'delivered') {
        toast.success('✅ Order delivered! Payment marked as Paid — Revenue updated.', { duration: 4000 });
      } else {
        ...
        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-10">
            <div className="bg-white p-5 rounded-sm shadow-sm border-l-4 border-blue-500 flex flex-col gap-1">
              ...
              <h2 className="text-3xl font-black text-gray-900">{stats.productCount}</h2>
            </div>
            <div className="bg-white p-5 rounded-sm shadow-sm border-l-4 border-green-500 flex flex-col gap-1">
              ...
              <h2 className="text-2xl font-black text-green-600">₹{stats.monthlyRevenue?.toFixed(2)}</h2>
            </div>
            <div className="bg-white p-5 rounded-sm shadow-sm border-l-4 border-teal-500 flex flex-col gap-1">
              ...
              <h2 className="text-2xl font-black text-teal-600">₹{stats.totalRevenue?.toFixed(2)}</h2>
            </div>
            <div className="bg-white p-5 rounded-sm shadow-sm border-l-4 border-orange-500 flex flex-col gap-1">
              ...
              <h2 className="text-3xl font-black text-gray-900">{stats.orderCount}</h2>
            </div>
            <div className="bg-white p-5 rounded-sm shadow-sm border-l-4 border-purple-500 flex flex-col gap-1">
              <p className="text-xs font-bold text-gray-500 uppercase flex items-center gap-1">
                <Users className="w-3 h-3" /> Users
              </p>
              <h2 className="text-3xl font-black text-gray-900">{stats.userCount}</h2>
            </div>
          </div>
        )}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div className="md:col-span-2 lg:col-span-3">
                          ...
                          <div className="flex gap-2 mb-3">
                            ...
                            <button
                              type="button"
                              onClick={() => { setImageMode('upload'); setValue('images', ''); setUploadedImageUrl(''); 
  setImagePreview('');  }}
                              className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded border transition-colors ${
                                imageMode === 'upload'
                                  ? 'bg-[#febd69] border-[#f2a14e] text-black'
                                ...
                                >
                                  {isUploading ? (
                                    ...
                                    <div className="flex flex-col items-center gap-1 text-green-600">
                                      ...
                                      <span className="text-xs text-gray-500 truncate max-w-xs">{uploadedImageUrl}</span>
                                      <span className="text-xs text-blue-500 mt-1">Click to change</span>
                                    </div>
                                  ) : (
                                    <div className="flex flex-col items-center gap-2 text-gray-400">
                                      <Upload className="w-6 h-6" />
                                      <span className="text-sm">Click to select image from PC</span>
                      ...
                      >
                        {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : (editingId ? <Save className="w-5 h-5" /> : <Plus className="w-5 h-5" />)}
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200">
                      <th className="px-6 py-4 text-xs font-bold uppercase text-gray-600">Product</th>
                      <th className="px-6 py-4 text-xs font-bold uppercase text-gray-600">Category</th>
                      <th className="px-6 py-4 text-xs font-bold uppercase text-gray-600">Price</th>
                      <th className="px-6 py-4 text-xs font-bold uppercase text-gray-600">Stock</th>
                      <th className="px-6 py-4 text-xs font-bold uppercase text-gray-600 text-right">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {isFetching ? (
                      <tr><td colSpan={5} className="text-center py-20"><Loader2 className="w-10 h-10 animate-spin mx-auto text-gray-300" /></td></tr>
                    ) : products.length === 0 ? (
                      <tr><td colSpan={5} className="text-center py-20 text-gray-500">No products found in inventory.</td></tr>
            ...
            <div className="bg-white rounded shadow-sm border border-gray-200 overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200">
                      <th className="px-5 py-4 text-xs font-bold uppercase text-gray-600">Order ID</th>
                      <th className="px-5 py-4 text-xs font-bold uppercase text-gray-600">Customer</th>
                      <th className="px-5 py-4 text-xs font-bold uppercase text-gray-600">Items</th>
                      <th className="px-5 py-4 text-xs font-bold uppercase text-gray-600">Amount</th>
                      <th className="px-5 py-4 text-xs font-bold uppercase text-gray-600">Payment</th>
                      <th className="px-5 py-4 text-xs font-bold uppercase text-gray-600">Status</th>
                      <th className="px-5 py-4 text-xs font-bold uppercase text-gray-600">Date</th>
                      <th className="px-5 py-4 text-xs font-bold uppercase text-gray-600">Update</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {isFetchingOrders ? (
                      <tr><td colSpan={8} className="text-center py-20"><Loader2 className="w-10 h-10 animate-spin mx-auto text-gray-300" /></td></tr>
                    ) : orders.length === 0 ? (
                      <tr><td colSpan={8} className="text-center py-20 text-gray-500">No orders placed yet.</td></tr>
                    ) : orders.map((order) => (
                      <tr key={order._id} className="hover:bg-gray-50 transition-colors text-sm">
                        ...
                        <td className="px-5 py-4">
                          <div className="font-bold text-gray-900">{order.user?.name || 'N/A'}</div>
                          <div className="text-xs text-gray-500">{order.user?.email}</div>
                        </td>
                        <td className="px-5 py-4">
                          <div className="flex flex-col gap-0.5">
                            {order.items?.slice(0, 2).map((item: any, i: number) => (
                              <span key={i} className="text-xs text-gray-600 line-clamp-1">
                        ...
                        <td className="px-5 py-4 font-bold text-gray-900">
                          <div className="flex items-center gap-1">
                            ...
                            {order.paymentStatus === 'paid' && (
                              <span title="Revenue counted" className="text-green-500 text-xs">✓</span>
                            )}
                          </div>
                        </td>
                        <td className="px-5 py-4">
                          <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${getPaymentBadge(order.paymentStatus)}`}>
                            {order.paymentStatus === 'paid' ? '✓ Paid' : order.paymentStatus}
                          </span>
                        </td>
                        <td className="px-5 py-4">
                          <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${getStatusColor(order.orderStatus)}`}>
                            {order.orderStatus}
                          </span>
                        </td>
