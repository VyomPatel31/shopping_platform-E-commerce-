...
async function testAPI() {
  ...
  try {
    ...
    const sendOtp = await fetch(`${BASE_URL}/auth/send-otp`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    const verifyOtp = await fetch(`${BASE_URL}/auth/verify-otp`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (loginRes.success && sendOtp.success && verifyOtp.message === 'INVALID_OTP') {
