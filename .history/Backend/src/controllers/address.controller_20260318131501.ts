...
// add address
export const addAddressController = async (req: Request, res: Response): Promise<void> => {
  try {
...
// set default address
export const setDefaultAddressController = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params
    ...
    res.status(httpStatus.OK).json(
      buildResponse(httpStatus.OK, { message: 'Default address updated', response: address })
    )
  } catch (err) {
    handleError(res, err)
  }
